export const homeObjOne = {
    id: 'about',
    lightBg: false,
    lightText: true,
    lightTextDesc: true,
    topLine: 'Who is High Metabolism?',
    headline: "Ivan's Story",
    description: 'Ivan started his fitness journey in 2016. During his freshman year  ',
    buttonLabel: 'Learn More',
    imgStart: false, /*switch image - textfirst */
    img: require('../../images/ivan-1.jpg'),
    alt: 'Profile Image',
    dark: true,
    primary: true,
    darkText: false
}

export const homeObjTwo = {
    id: 'music',
    lightBg: true,
    lightText: false,
    lightTextDesc: false,
    topLine: 'Music',
    headline: 'Now Streaming',
    description: 'Soundcloud and Spotify',
    buttonLabel: 'Listen Now',
    imgStart: true, /*switch image - textfirst */
    img: require('../../images/dreema-photo-2.jpg'),
    alt: 'Concert Image',
    dark: false,
    primary: false,
    darkText: true
}

export const homeObjThree = {
    id: 'about',
    lightBg: false,
    lightText: true,
    lightTextDesc: true,
    topLine: 'Who is Dreema?',
    headline: 'An Artist Like No Other',
    description: 'Dreema is an EDM Producer and DJ from Vancouver Canada, who likes vampires, booze, and having sex with many women',
    buttonLabel: 'Learn More',
    imgStart: false, /*switch image - textfirst */
    img: require('../../images/dreema-photo-3.jpg'),
    alt: 'Car',
    dark: true,
    primary: true,
    darkText: false
}